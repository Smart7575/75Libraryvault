...
export default function ProfileView({ user, books, isDarkMode }: ProfileViewProps) {
  ...
  const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 800 * 1024) {
        alert(t('profile.largePhotoError') || "Deze foto is te groot. Selecteer een foto van maximaal 800KB.");
        return;
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(t('profile.saveError') || "Er is een fout opgetreden bij het opslaan. Probeer het opnieuw.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const goalBooks = books.filter(b => {
    ...
    <div className="max-w-4xl mx-auto space-y-12 pb-20">
      ...
      <AnimatePresence>
        {showSuccess && (
          ...
          >
            <Check size={14} /> {t('profile.profileUpdated') || 'Profiel succesvol bijgewerkt'}
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {/* Header Info */}
      <div className="px-8 pt-8 flex items-end justify-between relative z-10">
        <div className="flex items-end gap-6 text-left">
          <div className="relative group/avatar">
            ...
            {isEditing && (
              ...
              >
                <span className="text-[10px] text-white font-bold uppercase tracking-widest px-2 text-center">{t('profile.uploadPhoto') || 'Foto Uploaden'}</span>
              </div>
            )}
          </div>
          <div className="pb-2">
            <h1 className="text-4xl font-serif font-black tracking-tight flex items-center gap-3">
              {isEditing ? (
                <input 
                  ...
                  placeholder={t('profile.displayNamePlaceholder') || "Naam"}
                />
              ) : (
        ...
        <div className="pb-4">
          {isEditing ? (
            <div className="flex gap-2">
              ...
              >
                {isSaving ? (t('profile.saving') || 'Bezig...') : <><Save size={12} /> {t('profile.save') || 'Opslaan'}</>}
              </button>
            </div>
          ) : (
            <button 
            ...
            >
              {t('profile.editProfile') || 'Profiel Bewerken'}
            </button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-12 px-8">
        ...
        <div className="md:col-span-1 space-y-8">
          <section className="space-y-4">
            <h3 className={cn("text-xs font-bold uppercase tracking-[0.2em] border-b pb-2 italic", isDarkMode ? "text-editorial-accent-bright border-zinc-800" : "text-editorial-accent border-editorial-border")}>{t('profile.personal') || 'Persoonlijk'}</h3>
            <div className="space-y-4">
              <InfoRow 
                icon={<Calendar size={14} />} 
                label={t('profile.birthDate') || "Geboortedatum"} 
                value={formData.birthDate || (t('profile.notSpecified') || 'Niet opgegeven')} 
                isEditing={isEditing}
              ...
              <InfoRow 
                icon={<MapPin size={14} />} 
                label={t('profile.city') || "Woonplaats"} 
                value={formData.city || (t('profile.notSpecified') || 'Niet opgegeven')} 
                isEditing={isEditing}
              ...
              <InfoRow 
                icon={<Globe size={14} />} 
                label={t('profile.country') || "Land"} 
                value={formData.country || (t('profile.notSpecified') || 'Niet opgegeven')} 
                isEditing={isEditing}
          ...
          <section className="space-y-4">
            <h3 className={cn("text-xs font-bold uppercase tracking-[0.2em] border-b pb-2 italic", isDarkMode ? "text-editorial-accent-bright border-zinc-800" : "text-editorial-accent border-editorial-border")}>{t('social.title') || 'Social'}</h3>
            <div className="space-y-4">
              <InfoRow 
          ...
          <section className="space-y-4">
            <h3 className={cn("text-xs font-bold uppercase tracking-[0.2em] border-b pb-2 italic", isDarkMode ? "text-editorial-accent-bright border-zinc-800" : "text-editorial-accent border-editorial-border")}>{t('profile.goalsAndStats')}</h3>
            <div className="space-y-6">
              <div>
                <div className={cn("flex items-center gap-3 mb-1 px-1", isDarkMode ? "text-white/40" : "text-black/50")}>
                  <Target size={14} />
                  <span className="text-[10px] font-bold uppercase tracking-widest">{t('profile.readingGoal') || 'Leesdoel'} 2026</span>
                </div>
                {isEditing ? (
                  <input 
                    type="number" 
                  ...
                  <div className={cn("px-6 py-4 border", isDarkMode ? "bg-zinc-950 border-zinc-800" : "bg-white border-editorial-border")}>
                    <div className="flex justify-between items-baseline mb-2">
                      <span className="text-2xl font-serif font-black">{goalBooks.length}</span>
                      <span className={cn("text-xs font-serif italic", isDarkMode ? "text-white" : "text-black/40")}>{t('profile.ofBooks', { count: Number(profile?.readingGoal || 0) })}</span>
                    </div>
                    <div className={cn("h-1 w-full", isDarkMode ? "bg-zinc-900" : "bg-neutral-100")}>
                      <div 
                         ...
                         style={{ width: `${Math.min(100, (goalBooks.length / (profile?.readingGoal || 1)) * 100)}%` }}
                      ></div>
                    </div>
                  </div>
                )}
              ...
              <div className="grid grid-cols-2 gap-4 text-center">
                <div className={cn("border p-4", isDarkMode ? "bg-zinc-950 border-zinc-800" : "bg-white border-editorial-border")}>
                  <span className={cn("block text-[8px] font-bold uppercase tracking-widest mb-1 italic", isDarkMode ? "text-white" : "text-black/40")}>{t('profile.totalPagesRead')}</span>
                  <span className="text-lg font-serif font-black">{totalPagesRead.toLocaleString()}</span>
                </div>
                <div className={cn("border p-4", isDarkMode ? "bg-zinc-950 border-zinc-800" : "bg-white border-editorial-border")}>
                  <span className={cn("block text-[8px] font-bold uppercase tracking-widest mb-1 italic", isDarkMode ? "text-white" : "text-black/40")}>{t('profile.avgSpeed')}</span>
        ...
        <div className="md:col-span-2 space-y-12">
          <section className="space-y-6">
            <h3 className={cn("text-xs font-bold uppercase tracking-[0.2em] border-b pb-2 italic", isDarkMode ? "text-editorial-accent-bright border-zinc-800" : "text-editorial-accent border-editorial-border")}>{t('profile.selectedBooks') || 'Geselecteerde Boeken'}</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
              ...
              <div className="space-y-3">
                <div className={cn("flex items-center gap-2 px-1", isDarkMode ? "text-white/40" : "text-black/50")}>
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-widest">{t('profile.favoriteBook') || 'Favoriete Boek'}</span>
                </div>
                {isEditing ? (
                  <button 
                  ...
                  >
                    {selectedFavorite ? (
                      ...
                      <>
                        ...
                        <p className={cn("text-[10px] font-bold uppercase tracking-widest", isDarkMode ? "text-white/20" : "text-black/40")}>{t('profile.chooseABook') || 'Kies een boek'}</p>
                      </>
                    )}
                  </button>
                ) : (
                  <div className={cn("border p-4 shadow-sm group", isDarkMode ? "bg-zinc-900 border-zinc-800" : "bg-white border-editorial-border")}>
                    {selectedFavorite ? (
                      ...
                      <p className={cn("text-xs italic py-8 text-center uppercase tracking-widest", isDarkMode ? "text-white" : "text-black/30")}>{t('profile.noFavoriteSelected') || 'Geen favoriet geselecteerd'}</p>
                    )}
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3">
                <div className={cn("flex items-center gap-2 px-1", isDarkMode ? "text-white/40" : "text-black/50")}>
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-widest">{t('profile.currentlyReading') || 'Nu aan het lezen'}</span>
                </div>
                {isEditing ? (
                  <button 
                       ...
                       <div className="flex gap-4">
                         ...
                         <div className="flex-1 min-w-0 pt-2">
                           ...
                           <div className={cn("mt-3 font-bold text-[8px] uppercase tracking-widest", isDarkMode ? "text-editorial-accent-bright" : "text-editorial-accent")}>
                             {t('profile.currentlyReadingBadge')}
                           </div>
                         </div>
                       </div>
                    ) : (
                      <p className={cn("text-xs italic py-8 text-center uppercase tracking-widest", isDarkMode ? "text-white" : "text-black/30")}>{t('profile.noBookSelected') || 'Geen boek geselecteerd'}</p>
                    )}
                  </div>
                )}
              </div>
            ...
            >
              <div className={cn("p-6 border-b flex items-center justify-between", isDarkMode ? "border-zinc-800" : "border-editorial-border")}>
                <h3 className="text-xl font-serif font-black uppercase tracking-tight italic leading-none">
                  {showBookSelector === 'favorite' ? (t('profile.selectFavorite') || 'Selecteer Favoriet') : (t('profile.selectBook') || 'Selecteer Boek')}
                </h3>
              ...
              <div className={cn("p-4 border-b flex items-center gap-3 transition-colors", isDarkMode ? "bg-zinc-950 border-zinc-800" : "bg-white border-editorial-border")}>
                ...
                <input 
                  ...
                  placeholder={t('profile.searchInCollection') || "Zoek in collectie..."}
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-2">
                ...
                {filteredBooks.length === 0 && (
                  <p className={cn("text-center py-12 text-sm italic", isDarkMode ? "text-white/20" : "text-black/30")}>{t('profile.noBooksFound') || 'Geen boeken gevonden'}</p>
                )}
              </div>
            </motion.div>
          </div>
